...
export default function LandingPage() {
  ...
  return (
    <div className="overflow-hidden bg-[#f7efe6]">
      <section className="mx-auto grid max-w-7xl gap-10 px-4 py-16 sm:px-6 lg:grid-cols-[1.2fr_0.8fr] lg:px-8 lg:py-24">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} data-reveal>
          <p className="mb-5 text-xs uppercase tracking-[0.45em] text-[#7ED957]">{profile.nickname} • {profile.currentRole}</p>
          ...
          <div className="mt-8 flex flex-wrap gap-4">
            <Link to="/contact" className="rounded-full bg-[#151515] px-6 py-3 text-sm uppercase tracking-[0.3em] text-white transition hover:bg-[#7ED957] hover:text-[#151515]">
              Let&apos;s work together
            </Link>
            <a href={profile.resumeUrl} className="rounded-full border border-black/15 px-6 py-3 text-sm uppercase tracking-[0.3em] text-[#151515] transition hover:border-[#7ED957] hover:text-[#7ED957]">
              Download Resume
            </a>
          </div>
          <div className="mt-10 flex flex-wrap gap-4 text-sm uppercase tracking-[0.25em] text-black/60">
            <a href={profile.instagram} target="_blank" rel="noreferrer" className="flex items-center gap-2 hover:text-[#7ED957]"> <FiInstagram /> Instagram</a>
            <a href={`mailto:${profile.email}`} className="flex items-center gap-2 hover:text-[#7ED957]"> <FiMail /> Email</a>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7 }} className="relative" data-reveal>
          ...
          <div className="absolute -bottom-6 -left-4 rotate-[-6deg] rounded-full border border-black/10 bg-[#7ED957] px-4 py-2 text-xs uppercase tracking-[0.3em] text-[#151515] shadow-lg">
            {profile.availability}
          </div>
        </motion.div>
      </section>

      <section className="border-y border-black/10 bg-[#f2e7d5]" data-reveal>
      ...
      <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8" data-reveal>
        <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr]">
          <div className="rounded-[2.5rem] border border-black/10 bg-[#fefbf7] p-8 shadow-[14px_14px_0_rgba(0,0,0,0.08)]">
            <p className="text-xs uppercase tracking-[0.35em] text-[#7ED957]">About</p>
            ...
            <Link to="/about" className="mt-8 inline-flex items-center gap-2 text-sm uppercase tracking-[0.3em] text-[#151515] hover:text-[#7ED957]">Read Full Story <FiArrowRight /></Link>
          </div>
          <div className="grid gap-4 md:grid-cols-2">
            {services.slice(0, 4).map((service) => (
              <div key={service.title} className="rounded-[2rem] border border-black/10 bg-[#fdf6eb] p-6">
      ...
      <section className="mx-auto max-w-7xl px-4 pb-16 sm:px-6 lg:px-8" data-reveal>
        <div className="mb-8 flex items-end justify-between gap-4">
          <div>
            <p className="text-xs uppercase tracking-[0.35em] text-[#7ED957]">Selected Work</p>
            <h2 className="mt-2 text-3xl font-semibold uppercase tracking-[0.18em]">Projects ready to be replaced</h2>
          </div>
          <Link to="/projects" className="hidden text-sm uppercase tracking-[0.3em] hover:text-[#7ED957] md:block">View all</Link>
        </div>
        <div className="grid gap-6 md:grid-cols-3">
          {projects.map((project) => (
            <motion.article key={project.title} whileHover={{ y: -8, rotate: -1 }} className="rounded-[2rem] border border-black/10 bg-[#fdf6eb] p-6 shadow-[10px_10px_0_rgba(0,0,0,0.06)]">
              <p className="text-xs uppercase tracking-[0.3em] text-[#7ED957]">{project.category}</p>
              ...
              <div className="mt-6 h-44 rounded-[1.5rem] border border-dashed border-black/20 bg-[#f9f2e7] p-4">
