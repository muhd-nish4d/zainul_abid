        ...
        <div className="grid gap-6 lg:grid-cols-3">
          {projects.map((project, index) => (
            <motion.article key={project.title} initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} whileHover={{ y: -8, scale: 1.01 }} className="group overflow-hidden rounded-[2rem] border border-white/10 bg-white/5 backdrop-blur-xl">
              <div className="relative h-64 overflow-hidden bg-gradient-to-br from-[#ff7a1a]/20 via-transparent to-[#ffffff]/10 p-6">
                <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,_rgba(255,122,26,0.4),_transparent_65%)]" />
              ...
              <div className="p-6">
                <p className="text-sm leading-7 text-[#f5e9db]/70">{project.blurb}</p>
                <Link to="/projects" className="mt-6 inline-flex items-center gap-2 text-[0.72rem] uppercase tracking-[0.3em] text-[#f5e9db] transition hover:text-[#ff7a1a]">View Project <FiArrowRight /></Link>
              </div>
            </motion.article>
          ))}
        </div>
