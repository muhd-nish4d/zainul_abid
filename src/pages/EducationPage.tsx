  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8 lg:py-24">
      ...
      <div className="mt-10 grid gap-6 lg:grid-cols-2">
        {education.map((item) => (
          <div key={item.degree} className="rounded-[2.5rem] border border-black/10 bg-[#fdf6eb] p-8">
            <p className="text-xs uppercase tracking-[0.35em] text-[#7ED957]">{item.period}</p>
        ...
        {certificates.map((item) => (
          <div key={item.title} className="rounded-[2.5rem] border border-black/10 bg-[#fefbf7] p-8">
            <p className="text-xs uppercase tracking-[0.35em] text-[#7ED957]">Certificate</p>
